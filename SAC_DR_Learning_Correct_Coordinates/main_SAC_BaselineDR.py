...
### Importing the Actor Class 
from actor import DiagGaussianActor
### Importing the SoftActor Critic Class
from sac import SACAgent
### Importing the Critic Class
...
train_reward_filename = os.path.join(base_log_dir, 'Dec19th_SAC_DRCollisionchecking_training_rewards_whole.txt')

end_effector_position_filename = os.path.join(base_log_dir, 'Dec19th_SAC_DREffector_trajectory_baseline.txt')
...
def train(args, env, model):
    ...
    if args.pre_train:
        try:
            base_path = "/home/robocupathome/rosur5_ws/src/contexualaffordance/DRLMODELSACSAC/8_12_2024/epoch_1120/"
            actor_path = os.path.join(base_path, "model_epoch_1120_actor.pth")
            critic_path = os.path.join(base_path, "model_epoch_1120_critic.pth")
            optim_a_path = os.path.join(base_path, "model_epoch_1120_actor_optimizer.pth")
            optim_c_path = os.path.join(base_path, "model_epoch_1120_critic_optimizer.pth")
    ...
    replay_buffer = ReplayBuffer(state_dim= env.state_dim, action_dim = env.action_dim,
                                 max_size=int(1e6))
    ...
    for epoch in range(0, 2000):
        ...
        while not_done:
            if args.model_name == 'SAC':
                ...
                action = (model.act(state_clipped, sample=False)).clip(-max_action, max_action)
                ...
                if total_num_of_steps  >= args.random_exploration:
                    ...
                    model.update(replay_buffer, episode_timesteps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_date', default='/19_12_2024')
    #Folder stored with trained model weights, which are used for transfer learning
    parser.add_argument('--model_date_', default='/19_12_2024')
    ...
    # ###Creating a object for Critic
    # critic_model = DoubleQCritic(obs_dim=env.state_dim, action_dim=env.action_dim, hidden_dim=1024,hidden_depth=2)

    # ###Creating a object for ACtor
    # actor_model = DiagGaussianActor(obs_dim=7,action_dim=6,hidden_dim=1024, 
