...
###GOAL = [-0.525000,0.00,1.684007,0.00,0.00,0.00]

# GOAL = [-0.116,0.00,0.854007,0.00,0.00,0.00]
GOAL = [-0.525000,0.00,0.854007,0.00,0.00,0.00]


# INIT = [0.0, -1.571, 1.571, -1.571, 0.0, 0.0]

INIT = [np.pi, -np.pi/2, np.pi/2, -np.pi/2, -np.pi/2, 0.0]
...
collision_contact_filename = os.path.join(base_log_dir, 'Dec19th_Test_Collision_Contact.txt')
...
class StateValidity():
    ...
    def checkCollision(self):
        ...
        result_of_collision = self.getStateValidity()

        if result_of_collision.valid:
            rospy.loginfo('robot not in collision, all ok!')
        else:
            rospy.logwarn('robot in collision')

            #### storing the contacts
            collision_objects = result_of_collision.contacts
            print("The Contacted Objects are: ", collision_objects)
           
           ### storing the collided objects
            with open(collision_contact_filename, "a") as file:
             file.write(f"Collision:{collision_objects}\n")
...
#### Function for Publishing block position on RViz
def publish_block(pose, color=RVizColors.BLUE, block_size=0.1):
    ...
    marker.ns = "block_marker"
    marker.id = 0  # Unique ID for the marker
    marker.type = Marker.CUBE  # Marker type (Cube)
    ...
    if color == RVizColors.BLUE:
        marker.color.r = 0.0
        marker.color.g = 0.0
        marker.color.b = 1.0
    elif color == RVizColors.RED:
        marker.color.r = 1.0
        marker.color.g = 0.0
        marker.color.b = 0.0
    elif color == RVizColors.GREEN:
        marker.color.r = 0.0
        marker.color.g = 1.0
        marker.color.b = 0.0
    elif color == RVizColors.WHITE:
        marker.color.r = 1.0
        marker.color.g = 1.0
        marker.color.b = 1.0
    ...
    rospy.loginfo(f"Published block marker at pose ({pose.position.x}, {pose.position.y}, {pose.position.z})")

    # Keep the node alive briefly to ensure the marker appears in RViz
    rospy.sleep(1)
...
class Ur5():
    ...
    def __init__(self, init_joints=INIT, goal_pose=GOAL, duration=DURATION, render_mode=None):
        ...
        ### object is generated
        self.object_pos = self.target_generate()
    ...
    def add_table_to_scene(self):
        ...
        self.scene.add_box("table", table_pose,
                           size=(1.77, 0.577, 0.03))  # Length: 1270 mm, Width: 577 mm, Height: 800 mm
        rospy.loginfo(
            "Table added to MoveIt planning scene with updated dimensions: 1770x577x800 mm and height 800 mm")
    ...
    def retricting_angles(self, action):
    ...
    # #### Function to normalize values between -0.2 t0 0.2
    # def normalize(values, old_min=-1, old_max=1, new_min=-0.2, new_max=0.2):
    #   return [((x - old_min) / (old_max - old_min)) * (new_max - new_min) + new_min for x in values]
    ...
    def step(self, action, object_position, timestep):
        ...
        #     action_clipped[i] = ((action[i] - (-1))/(1 - (-1))) * (0.2 - (-0.2)) + (-0.2)
        ...
        self.retricting_angles(action_sent)
        ...
        print("Current position of the end-effector is: ", position)

        print("Object position is", object_position[:3])
    ...
    def reset(self):
        ...
        print("Reset Start path is: ", start)
        self.retricting_angles(start)
        print("After Reset Start path is: ", start)

        #stop = [0.0, -1.571, 1.571, -1.571, 0.0, 0.0]
        stop = [np.pi, -np.pi/2, np.pi/2, -np.pi/2, -np.pi/2, 0.0]

        print("Reset Stop path is ", stop)

        # stop = [0.7, 0.00, 0.854007, 0.00, 0.00, 0.00]

        ###creating an uniform spaced path
        path = np.linspace(start, stop, num=10)

        print ("Reset path is: ", path)
        ...
        print('The position of the object is: ', object_position[:3])

        block_pose = Pose()
        block_pose.position.x = GOAL[0]
        block_pose.position.y = GOAL[1]
        block_pose.position.z = GOAL[2]
        block_pose.orientation.x = GOAL[3]
        block_pose.orientation.y = GOAL[4]
        block_pose.orientation.z = GOAL[5]
        block_pose.orientation.w = 1.0

        publish_block(block_pose, color=RVizColors.BLUE, block_size=0.1)
        ...
        # end = self.checking_end_effector_pos(object_position, position, 0)

        return state_obtained, object_position, position
   ...
   ##### Function for Penalizing and Rewarding the Agent
    ...
    def get_pos(self):
        ...
        current_pose = self.group.get_current_pose('wrist_3_link').pose
