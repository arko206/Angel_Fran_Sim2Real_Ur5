...
train_reward_filename = os.path.join(base_log_dir, 'Dec5th_Collisionchecking_training_rewards_whole.txt')

end_effector_position_filename = os.path.join(base_log_dir, 'Dec5th_Effector_trajectory_baseline.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_date', default='/5_12_2024')
    #Folder stored with trained model weights, which are used for transfer learning
    parser.add_argument('--model_date_', default='/5_12_2024')
    ...
    if args.model_name == 'SAC': model = SACAgent(obs_dim = env.state_dim, action_dim = env.action_dim, action_range=[-0.2, 0.2],
      ...
      critic_betas=[0.9, 0.999], critic_tau=0.005,critic_target_update_frequency=2,batch_size=1024,
      learnable_temperature = True, hidden_dim=1024,hidden_depth=2, log_std_bounds=[-5,2])
